...
export function construirePromptAppFinale(uc: UseCase): string {
  ...
  L.push('');
  L.push('## SÉCURITÉ « BY DESIGN » (impératif — référentiel OWASP ASVS Niveau 2)');
  L.push('La sécurité est intégrée dès la conception, pas ajoutée après. Applique :');
  L.push('- Secure defaults : HTTPS forcé, cookies httpOnly+Secure+SameSite, CORS en liste blanche,');
  L.push('  en-têtes de sécurité (CSP, HSTS, X-Content-Type-Options) actifs d\'origine.');
  L.push('- Entrées validées côté serveur (schémas typés) ; sorties échappées (anti-XSS).');
  L.push('- Requêtes paramétrées / ORM partout (anti-injection SQL).');
  L.push('- AuthZ vérifiée à CHAQUE endpoint, objet par objet (anti-IDOR) ; refus par défaut.');
  L.push('- Mots de passe hashés (argon2/bcrypt), tokens courts + refresh, anti-brute-force (rate-limit login).');
  L.push('- Gestion d\'erreurs « fail securely » : en cas de doute refuser ; aucun détail technique fuité.');
  L.push('- Secrets hors code (.env / Key Vault), jamais dans les logs. Logs de sécurité (login, accès refusés).');
